...
class State:
    ...
    def clang_tidy_run(self,
        ...
        compile_commands_json = [
            {
             ...
             "file": "{}".format(executable_filename)
            }
        ]
        ...
        ex = execute(['clang-tidy', '-p', COMPILE_COMMAND_JSON_FILENAME, '-quiet', '-header-filter=.*',
                      '-export-fixes={}'.format(CLANG_TIDY_YAML_FILENAME), 
                      ' '.join(implementation_file_names) if implementation_file_names is not None else ' ',
                      executable_filename],
                     timeout=timeout)
